...
def get_protocol_tvl(protocol):
    ...
    if response.status_code == 200:
        try:
            tvl_data = response.json()["chainTvls"]["staking"]["tvl"]
        except KeyError:
            print(f"No staking data found for {protocol}")
            return []
...
def main():
    ...
    with open(f"{category}_staking.csv", "w", newline="") as csvfile:
            ...
            for protocol in protocols:
                for d in data[protocol]:
                    if d["date"] == date:
                        ...
                        break
            writer.writerow(row)
        print(f"Data written to {category}_staking.csv")
