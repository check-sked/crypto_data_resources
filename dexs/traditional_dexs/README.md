# traditional_dex_volumes

## Files and Folders Overview

### dex_names_volume_change.py

- Returns name of DEXs, their display names, and change in volume over 1-day, 7-day, and 30-day periods.
- Change cells represent % change based on time period. For example -32.15 is the equivalent of -32.15% change over the specified period.
- This file should be regenerated on a semi-regular basis to capture all new DEXs added to the API.
- See [here](https://github.com/check-sked/crypto_data_resources/blob/main/csv_examples/dexs/dex_names_and_volumes.csv) for example of file returned.

--

### dex_volume_by_protocol.py

- Returns historical volume of DEXs.
- User inputs protocol name and daily or cumulative volume via in terminal prompts.
- Refer to [Protocols](#protocols) for available DEXs or [this file](https://github.com/check-sked/crypto_data_resources/blob/main/csv_examples/dexs/dex_names_and_volumes.csv).
- See [here](https://github.com/check-sked/crypto_data_resources/blob/main/csv_examples/dexs/Uniswap_daily_volume.csv) for example of file returned.

--

### dex_volume_by_chain.py

-
- See [here](https://github.com/check-sked/crypto_data_resources/blob/main/csv_examples/dexs/Tezos_volume_highlights.csv) for example of file returned.

## Protocols

**DEXs Available**

**#**

- 10KSwap
- 3xcalibur
- 4Swap

**A**

- ABcDeFX
- Acala Swap
- Aequinox
- Agni Finance
- AirSwap
- Aldrin
- ALEX
- Algofi Swap
- AlienFi
- Alita Finance
- Ambient
- AmpleSwap
- ApeSwap AMM
- Arbitrum Exchange V2
- Arbitrum Exchange V3
- ArcherSwap
- Archly Finance
- Arctic
- Ashswap
- Astroport Classic
- AstroSwap
- Auragi Finance
- AuraSwap
- AutoShark
- AUX Exchange
- Axial

**B**

- BabyDogeSwap
- BabySwap
- Balancer V1
- Balancer V2
- Bancor V2.1
- Bancor V3
- BeamSwap Classic
- BeamSwap Stable AMM
- Beethoven X
- BenSwap
- Bisq
- BiSwap V2
- Blue Planet
- BXH

**C**

- Camelot V2
- Camelot V3
- Canto Dex
- Capricorn Finance
- CarbonSwap
- Cetus
- ChampagneSwap
- CherrySwap
- Chronos
- ClaimSwap V1
- Clipper
- Clober
- Cometh
- Complus Network
- Concordex
- Cone
- Covo V2
- Crema Finance
- Crodex
- CroSwap
- CryptoSwap
- Curve DEX

**D**

- DAO Swap
- Darkness
- DeepBook
- Defi Kingdoms
- Defi Swap
- DefiBox
- DefiChain DEX
- DefiPlaza
- Demex
- DerpDEX
- Dexalot
- Dexter
- DFX Finance
- Dfyn Network
- Dinosaur Eggs
- DODO
- Dove Swap V3
- DPEX
- Dystopia

**E**

- E3
- EchoDEX
- El Dorado Exchange
- Elk
- Ellipsis Finance
- EmpireDEX
- Energiswap
- Equalizer Exchange
- Equilibre
- Equity
- ExinSwap

**F**

- FairySwap V1
- Fathom Dex
- Ferro
- Flamingo Finance
- FlowX Finance
- Forge
- Frax Swap
- Fulcrom
- FusionX V2
- FusionX V3
- FX Swap
- FXDX

**G**

- Gin Finance
- Glide Finance
- GMX
- GooseFX
- Gravis Finance
- Gravity Finance

**H**

- Hadouken AMM
- HakuSwap
- Hashflow
- HeliSwap
- Helix
- Heraswap
- Hermes Protocol
- HiveSwap
- Honeyswap
- HorizonDEX
- Huckleberry AMM
- Humble Defi
- Hummus Exchange
- Hydradex V2
- Hydradex V3
- HyperJump

**I**

- IcecreamSwap
- ICPSwap
- Increment Swap
- Integral
- iZiSwap

**J**

- JediSwap
- JetSwap
- Joe V2
- Joe V2.1
- JSwap
- Junoswap

**K**

- Kaidex
- Karura Swap
- Katana DEX
- Kava Swap
- KlaySwap
- KLEX Finance
- KnightSwap Finance
- Kokonut Swap
- Koyo Finance
- KuSwap
- KyberSwap Classic
- KyberSwap Elastic
- KyotoSwap

**L**

- LeonicornSwap
- Level Finance
- Levinswap
- Lif3 Swap
- Lifinity
- Lighter
- LiquidSwap
- Litx
- LuaSwap
- LumenSwap

**M**

- Maia V3
- MakiSwap
- MantisSwap
- Maverick Protocol
- MDEX
- Megaton Finance
- MERLIN DEX
- Meshswap
- MetaTdex
- Metavault.Trade
- Metropolis
- MiaSwap
- Mimo
- Minswap
- MistSwap
- MM Finance Arbitrum
- MM Stableswap Polygon
- Mobius Money
- MojitoSwap
- Moon Swap
- MooniSwap
- Morpheus Swap
- Morphex
- MuesliSwap
- Mummy Finance
- Mute.io
- mySwap

**N**

- Native
- NetSwap
- Nomiswap

**O**

- OKCSwap
- OnePunchSwap
- OolongSwap
- OpenBook
- OpenLeverage
- OPX Finance
- OraiDEX
- Orca
- Orderly Network
- Osmosis DEX
- Oswap AMM

**P**

- Pact
- PadSwap
- Paint Swap
- PancakeSwap AMM
- PancakeSwap AMM V1
- PancakeSwap AMM V3
- PancakeSwap StableSwap
- Pandora Swap
- Pangea Swap
- Pangolin
- PayCash
- PearlFi
- PegaSys V2
- Penguin
- PheasantSwap
- PhotonSwap Finance
- PinkSwap
- Pinnako
- Platypus Finance
- Plenty
- Polkaswap
- Polycat
- PonytaSwap
- ProtoFi
- PulseX V1
- PulseX V2
- PYEswap

**Q**

- Quickswap Dex
- Quickswap V3
- QuipuSwap V1

**R**

- RadioShack
- Ramses V1
- Ramses V2
- Raydium
- Reax Swaps
- Ref Finance
- Rubicon

**S**

- Saber
- Saddle Finance
- Saros
- SatoshiCoreSwap
- SaucerSwap
- Serum
- SharkSwap
- Shell Protocol
- ShibaSwap
- SmarDex
- SmartDEX
- SMBSwap V2
- SMBSwap V3
- Solarbeam
- Solarflare
- SolidLizard
- Solidly
- Solidly V2
- SoulSwap
- Soy Finance
- SpaceDex
- Spartacus Exchange
- Spartan
- Sphynx
- SpiritSwap AMM
- SpookySwap
- StellarX
- StellaSwap V2
- StellaSwap V3
- Step Exchange
- Subzero ZSwap
- SUNSwap V1
- SUNSwap V2
- Surfswap Classic
- Surfswap Stable AMM
- Sushi Trident
- SushiSwap
- SushiSwap V3
- Swapline
- Swappi
- Swapr
- Synthetify
- Syrup Finance

**T**

- Tealswap
- TempleDAO Trade
- Terraswap
- Tethys AMM
- Tetu Swap
- ThalaSwap
- THENA FUSION
- Thena V1
- THORSwap
- Tinyman
- Titano Swych
- Tokenlon
- Tomb Swap
- Trader Joe DEX
- TTswap

**U**

- Ubeswap
- UltronSwap
- Unicly
- UniFi
- Uniswap V1
- Uniswap V2
- Uniswap V3

**V**

- VanSwap
- VaporDex
- Veax
- Velocimeter V2
- Velodrome V1
- Velodrome V2
- VeniceSwap
- Verse
- Vertex Protocol
- Vexchange
- ViperSwap
- Voltage
- VoltSwap V2
- Voodoo Trade
- Vortex Protocol
- VVS Finance

**W**

- WAGMI
- WagyuSwap
- WanSwap Dex
- WardenSwap
- Wavelength DAO
- WEMIX.FI AMM
- WhaleSwap
- WigoSwap
- WinerySwap
- WingRiders
- WingSwap
- Wojak Finance
- Wombat Exchange
- WOOFi Swap
- WX Network

**X**

- xExchange
- XSwap Protocol

**Y**

- YieldFields
- Yodeswap
- YokaiSwap
- Yoshi Exchange

**Z**

- ZigZag
- ZilSwap
- ZipSwap
- Zircon Gamma
- zkSwap Finance
- Zyberswap AMM
- ZyberSwap Stableswap
- Zyberswap V3
