...
for chain in chains:
    ...
    for item in chart_data:
        ...
        date_str = datetime.fromtimestamp(date).strftime("%Y-%m-%d %H:%M:%S") #%d-%m-%Y
...
with open(output_filename, mode="w", newline="") as csv_file:
    ...
    min_date = min([min(data, key=lambda x: x[0])[0] for data in chain_data.values()])
    max_date = max([max(data, key=lambda x: x[0])[0] for data in chain_data.values()])
    ...
    for date in sorted(all_dates):
        ...
        for chain in chains:
            chain_name = chain["name"].replace(" ", "-")
            if chain_name in chain_data and date in [data[0] for data in chain_data[chain_name]]:
                row.append([data[1] for data in chain_data[chain_name] if data[0] == date][0])
            else:
                row.append("")
        writer.writerow(row)

# Print a message to confirm that
print(f"TVL data saved to {output_filename}")