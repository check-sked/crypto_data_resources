...
with open('stablecoin_prices.csv', 'w', newline='') as csvfile:
    ...
    for date_prices in prices:
        ...
        date = datetime.datetime.fromtimestamp(date_prices['date']).strftime('%Y-%m-%d')
        ...
        for stablecoin in stablecoins:
            if stablecoin in date_prices['prices']:
                ...
                price = prev_prices.get(stablecoin, 0)  # Use null as the default value if the stablecoin has no previous price
            row.append(price)
